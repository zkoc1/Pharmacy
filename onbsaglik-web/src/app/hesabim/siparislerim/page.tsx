...
export default function SiparislerimSayfasi() {
  ...
  const handleCancel = async (id: string) => {
    ...
    try {
      ...
      if (data.success) {
        window.alert("Sipariş başarıyla iptal edildi ve iptal süreci başlatıldı.");
        fetchUserOrders();
      } else {
        window.alert(data.error || "İptal işlemi başarısız.");
      }
    } catch (e) {
      window.alert("Sistemsel bir hata oluştu.");
    }
